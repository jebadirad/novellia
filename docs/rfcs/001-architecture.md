...
PATCH supports changing a subset of top-level editable fields by merging with the current validated record. A supplied details object replaces the previous details object and must independently satisfy that type's schema. Pet and type cannot change on an existing medical record.
